...
| HTTP client (`@arc/core`) | native `fetch` + custom retry/timeout wrapper | n/a | HIGH |
...
| Validation | **Zod 4** for v0.1; consider `valibot` for `@weave/react` runtime if bundle size becomes an issue | `4.4.x` / `1.4.x` | HIGH |
| Client state (Weave Admin) | **Zustand** (sidebar UI state) + **TanStack Query 5** (server state / REST sync) | `5.0.x` / `5.100.x` | HIGH |
| Cache (`@arc/next`) | Next.js 16 `'use cache'` directive + `cacheTag` + `revalidateTag` (NOT `unstable_cache`) | n/a | HIGH |
...
|---|---|
| `params` / `searchParams` are now **Promises only** — synchronous access removed | `@arc/next` loaders must `await` |
...
| `'use cache'` directive + `cacheTag` is the **recommended** model; `unstable_cache` is legacy | `@arc/next` cache helpers should target `'use cache'` from day 1 |
...
### 1.2 `@arc/core` (framework runtime)
...
### 1.3 `@arc/next` (Next.js integration)
| Library | Version | Purpose | Rationale |
|---|---|---|---|
...
### 1.4 `@weave/react` (sections SDK)
| Library | Version | Purpose | Rationale |
|---|---|---|---|
| `zod` | `4.4.x` | `WeaveComponentSchema` validation | Same as `@arc/core` — share via workspace. Author schemas as `z.object(...)`, infer the TS type. |
| `react` | peer `>=19` | — | — |
### 1.5 `@weave/next` (Next.js section renderer)
| Library | Version | Purpose | Rationale |
|---|---|---|---|
| `@arc/next` | `workspace:*` | Cache helpers reused for page-config fetches | — |
| `@weave/react` | `workspace:*` | Schema types | — |
...
| Zod 4 | Valibot | When `@weave/react` schema validation lands in a customer-facing bundle and 15kB matters. v0.2 decision. |
...
| `<Money>`, `<Image>`, `<Video>` primitives | Either build equivalents in `@arc/next` or document `next/image` + Intl.NumberFormat patterns | LOW — recommend "document the pattern, don't ship the component" for v0.1. |
...
| Zod over Valibot for v0.1 | HIGH for v0.1; MEDIUM for v0.2 | DX wins now, bundle size may matter later in `@weave/react` |
