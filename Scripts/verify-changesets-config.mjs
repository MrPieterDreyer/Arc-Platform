...
if (!hasArcGroup) { console.error('[verify-changesets-config] fixed groups missing ["@arc/*"]'); failed += 1; }
if (!hasWeaveGroup) { console.error('[verify-changesets-config] fixed groups missing ["@weave/*"]'); failed += 1; }

if (cfg.access !== 'public') {
  console.error('[verify-changesets-config] access must be "public" (got ' + JSON.stringify(cfg.access) + ')');
  failed += 1;
}

const changelog = Array.isArray(cfg.changelog) ? cfg.changelog[0] : cfg.changelog;
if (changelog !== '@changesets/changelog-github') {
  console.error('[verify-changesets-config] changelog plugin must be "@changesets/changelog-github"');
  failed += 1;
}
