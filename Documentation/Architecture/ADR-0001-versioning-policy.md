...
Per PITFALLS P7, premature promotion to `1.0.0` of `@arc-platform/core` would freeze the wrong API before Customer Zero (LOFT Pro Shop) has validated it in production. There is external pressure to ship a `v1` for marketing credibility, but doing so trades long-term API correctness for a short-term optics win. The Hydrogen parallel is instructive: Shopify shipped Hydrogen `0.x` through two full design iterations before 1.0 — and avoided locking the RSC patterns until the App Router was stable.

Arc's first consuming project (LOFT Pro Shop) is a private monorepo that will exercise the full `@arc-platform/core` + `@arc-platform/next` + `@weave-platform/react` stack. Until that production signal exists, AND at least one external community project ships on Arc, the public API cannot be considered proven.
...
1. **Start at 1.0.0, semver strictly** — ship `@arc-platform/core@1.0.0` immediately; breaking changes require major bumps from day one
...
All public `@arc-platform/*` and `@weave-platform/*` packages stay at `0.x.y` until BOTH criteria are met:
...
Changesets `fixed` groups keep `@arc-platform/*` and `@weave-platform/*` in lockstep so the two SDKs always share a version number.
...
- Keeping `@arc-platform/*` and `@weave-platform/*` on the same version in `fixed` lockstep means an `@arc-platform/core` breaking change forces a `@weave-platform/react` minor bump even if `@weave` itself is unchanged.
...
- `.changeset/config.json` (plan 05) sets `fixed: [["@arc-platform/core", "@arc-platform/next"], ["@weave-platform/react", "@weave-platform/next"]]` to maintain lockstep versioning.
